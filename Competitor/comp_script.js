...
if (is_logged_in !== 'True') {
  ...
  const switch_to_client_button = document.querySelector('#comp');

  let valid_comp_names = ['ROYALxGIFTS'];
  let all_offers = [];
  ...
  // Fetch all the user data mentioned in the valid compnames list
  if (
    JSON.parse(localStorage.getItem('compnames')) &&
    JSON.parse(localStorage.getItem('compnames')).length != 0
  ) {
    valid_comp_names = JSON.parse(localStorage.getItem('compnames'));
  }
  ...
  let add_user = (e) => {
    ...
    valid_comp_names.push(username_input.value);
    display_user(username_input.value);
    get_offers_data_no_auth(username_input.value);

    localStorage.setItem('compnames', JSON.stringify(valid_comp_names));

    username_input.value = '';

    // console.log(interested_offers, valid_comp_names);
  };
  ...
  let remove_user = () => {
    ...
    let names_length = valid_comp_names.length;
    ...
    for (let name = 0; name < names_length; name++) {
      if (valid_comp_names[name] === user_name) {
        valid_comp_names.splice(name, 1);
  ...
  switch_to_client_button.addEventListener('click', switch_to_client);

  //Save the compnames to localStorage
  localStorage.setItem('compnames', JSON.stringify(valid_comp_names));
}
